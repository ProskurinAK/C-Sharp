...
namespace RandomForest
{
    class DecisionTree
    {
        ...
        private List<List<double>> Training_Sample = new List<List<double>>();  // Считанная из файла обучающая выборка
        /*
        ...
        public DecisionTree(int NumbOfModel)
        {
            ...
            while (Training_Sample.Count > 1) // Условие создания новых узлов дерева
            {
                (Training_Sample, OnePredicate) = Training();
        ...
        // ------------------------------------------------------------------------------------------------------------
        private void ReadTrainingSample()
        {
            // Функция чтения обучающей(бутстрэп) выборки из файла

            StreamReader Sr = new StreamReader($@"D:\Progi\C#\ML\EnsembleOfModels\Bagging\RandomForest\TrainingSamples\TrainingSample{NumbOfModel}.txt");

            int AmountOfRows = System.IO.File.ReadAllLines($@"D:\Progi\C#\ML\EnsembleOfModels\Bagging\RandomForest\TrainingSamples\TrainingSample{NumbOfModel}.txt").Length;
            ...
            for (int i = 0; i < AmountOfRows; i++)
            {
                ...
                else
                {
                    ...
                    Training_Sample.Add(RowInTraining_Sample);
                }
            }
        }
        // ------------------------------------------------------------------------------------------------------------
        private (List<List<double>>, List<List<double>>) Training()
        {
            ...
            for (int AmountOfFeature = 0; AmountOfFeature < Training_Sample[1].Count - 1; AmountOfFeature++)
            {
                ...
                for (int i = 0; i < Training_Sample.Count; i++)
                {
                    ...
                    for (int j = i + 1; j < Training_Sample.Count; j++)
                    {
                        if (Training_Sample[j][AmountOfFeature] < Training_Sample[MinIndex][AmountOfFeature])
                        {
                            MinIndex = j;
                        }
                    }
                    if (MinIndex != i)
                    {
                        for (int j = 0; j < Training_Sample[i].Count; j++)
                        {
                            TmpList.Add(Training_Sample[i][j]);
                        }

                        for (int j = 0; j < Training_Sample[i].Count; j++)
                        {
                            Training_Sample[i][j] = Training_Sample[MinIndex][j];
                        }

                        for (int j = 0; j < Training_Sample[i].Count; j++)
                        {
                            Training_Sample[MinIndex][j] = TmpList[j];
                        }
                    }
                }
                ...
                for (int i = 0; i < Training_Sample.Count; i++)
                {
                    VectorOfTarget.Add(Training_Sample[i][Training_Sample[i].Count - 1]);
                }
                ...
                for (int i = 0; i < VectorOfTarget.Count - 1; i++)
                {
                    if (VectorOfTarget[i] != VectorOfTarget[i + 1]) // Индекс джинни считается только в тех случаях, когда происходит изменение значения соседних целевых признаков 
                    {
                        ...
                        if (AllDeltaOfGiniIndex.Count == 0)
                        {
                            ...
                            if (DeltaOfGiniIndex == MinOfAllDeltaOfGiniIndex)
                            {
                                ...
                                if (GiniIndexForLeftSample > GiniIndexForRightSample)
                                {
                                    ...
                                    RowInPredicate.Add((Training_Sample[i][AmountOfFeature] + Training_Sample[i + 1][AmountOfFeature]) / 2);
                                    RowInPredicate.Add(1);
                                    RowInPredicate.Add(Training_Sample[i + 1][Training_Sample[i].Count - 1]);
                                ...
                                else if (GiniIndexForLeftSample < GiniIndexForRightSample)
                                {
                                    ...
                                    RowInPredicate.Add(Training_Sample[i][Training_Sample[i].Count - 1]);
                        ...
                        Console.WriteLine("Value in this index = " + Training_Sample[i][AmountOfFeature]);
                        //Console.WriteLine();
                    }
                }
                    ...
                    if (AllDeltaOfGiniIndex.Count == 1 || FlagSampleSplitIndex == true)
                    {
                        ...
                        if (IsLeftSample == true)
                        {
                            for (int i = 0; i <= SampleSplitIndex; i++)
                            {
                                ...
                                for (int j = 0; j < Training_Sample[i].Count; j++)
                                {
                                    RowInNewTrainingSample.Add(Training_Sample[i][j]);
                                }
                        ...
                        {
                            for (int i = SampleSplitIndex + 1; i < Training_Sample.Count; i++)
                            {
                                List<double> RowInNewTrainingSample = new List<double>();
        ...
        private void WritePredicatesToFile()
        {
            ...
            StreamWriter Sw = new StreamWriter($@"D:\Progi\C#\ML\EnsembleOfModels\Bagging\RandomForest\Predicates\Predicate{NumbOfModel}.txt", false);
